...
export const SocketProvider: React.FC<SocketProviderProps> = ({ children }) => {
  ...
  const connectSocket = useCallback(() => {
    ...
    const setupSocketEventHandlers = (socket: Socket) => {
      ...
      socket.on("order_status_updated", (order) => {
        ...
        window.dispatchEvent(
          new CustomEvent("orderStatusUpdated", { detail: order })
        );
      });
      ...
      socket.on("customer_joined_table", (data) => {
        ...
        window.dispatchEvent(
          new CustomEvent("tableStatusUpdated", {
      ...
      socket.on("session_started", (data) => {
        ...
        window.dispatchEvent(
          new CustomEvent("sessionStarted", { detail: data })
        );
      });
      ...
      socket.on("session_ended", (data) => {
        notification.info({
          ...
          description: `Session ended at Table ${data.tableId}${
            data.reason ? ` (${data.reason})` : ""
          }`,
          placement: "topRight",
          duration: 3,
      ...
      socket.on("customer_message", (data) => {
        ...
        window.dispatchEvent(
          new CustomEvent("customerMessage", { detail: data })
        );
      });
  ...
  const sendMessageToTable = useCallback(
