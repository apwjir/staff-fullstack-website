...
class AdminApiService {
  ...
  async toggleTableStatusManually(
    id: number,
    status: "AVAILABLE" | "OCCUPIED"
  ): Promise<Table> {
  ...
  async getTableWithStatus(id: number): Promise<
    Table & {
  ...
  async createMenuItemWithImage(
    menuItem: Omit<MenuItem, "id" | "photoUrl" | "photoId">,
    imageFile?: File
  ): Promise<MenuItem> {
    if (!imageFile) {
  ...
  async updateMenuItemWithImage(
    id: number,
    menuItem: Partial<Omit<MenuItem, "id" | "photoUrl" | "photoId">>,
    imageFile?: File
  ): Promise<MenuItem> {
    if (!imageFile) {
    ...
    if (menuItem.price !== undefined)
      formData.append("price", menuItem.price.toString());
    if (menuItem.foodtype !== undefined)
      formData.append("foodtype", menuItem.foodtype);
  ...
  async getDashboardStats(): Promise<{
    ...
    const todayBills = bills.filter(
      (bill) => new Date(bill.createdAt) >= today && bill.isPaid
    );
...
export const mapTableStatus = (
  backendStatus: "AVAILABLE" | "OCCUPIED"
): "available" | "occupied" => {
  return backendStatus.toLowerCase() as "available" | "occupied";
};

export const mapOrderStatus = (
  backendStatus: "PENDING" | "IN_PROGRESS" | "DONE" | "CANCELLED"
...
export const mapFrontendOrderStatus = (
  frontendStatus: "waiting" | "cooking" | "done"
